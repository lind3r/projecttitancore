...
| Compressed Log | `gen_compressed_log.py` | **Wood palette, not holy** — used as a tier-recipe ingredient for the Core. Vertical bark streaks + 2-px darkened border (the "compressed" tell). Lives outside `gen_building_block_texture.py` to keep the holy palette pure. |
