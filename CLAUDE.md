...
**Titan projection structure** — `scripts/gen_titan_structure.py`. Builds the holographic-statue voxel data emitted as `data/projecttitancore/structure/titan.json` plus cumulative front-elevation previews in `scripts/preview_titan/` (gitignored). Body is split into part-functions (`t1_plinth`, `t1_feet`, `t1_lower_legs`, `t2_upper_legs`, `t2_hips_belt`, `t3_waist`, `t3_chest`, `t3_arms`, `t4_neck_head`, `t4_helmet`, `t4_sword`) that each return `(x, y, z, color)` voxel tuples; `TIER_BUILDERS` aggregates them per tier. Coordinate frame: +X = titan's right (sword side), +Y = up, +Z = forward; origin = top of crafting beam (`y = corePos.y + BEAM_RENDER_HEIGHT`). Three colors only — `ivory`, `shadow`, `gold`, matching the holy palette. To tweak proportions: edit a part function, re-run, inspect the previews, then `deployToInstance`.
...
6. 🚧 **Titan projection** — translucent rotating holographic statue that grows tier-by-tier above the crafting beam. T1 = feet/lower legs, T2 = upper legs/hips/belt, T3 = torso/arms, T4 = head/helmet/sword. Each `TitanCoreRecipe` carries a `tier` field; on craft completion `TitanCoreBlockEntity.titanTier` bumps to the recipe's tier (capped at max-ever-seen). `TitanCoreRenderer.renderProjection` reads the BE's tier each frame and submits cumulative voxels via a custom `POSITION_COLOR` translucent `RenderType`. Voxel data: `data/projecttitancore/structure/titan.json`, generated by `scripts/gen_titan_structure.py`. Slow Y rotation (~24°/sec) and slow alpha breathe (~4s cycle) baked into the renderer. **Not yet wired:** the BE-side projection is render-only — no structure ever materializes as real blocks.
...
- **Crafting beam (and now the titan projection) cull when block is offscreen.** Despite `shouldRenderOffScreen=true` on the BER and a generous AABB on the BE, the beam + projection disappear the moment the core leaves the camera frustum (e.g. standing next to the core and tilting up — they go invisible as soon as the core slips off the bottom of the screen). Vanilla beacon stays visible from far away.
